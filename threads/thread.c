...
thread_unblock (struct thread *t) {
	...
	list_insert_ordered(&ready_list, &t->elem, cmp_thread_priority, NULL);	// ready_list에 스레드를 추가
...
thread_yield (void) {
	...
	if (curr != idle_thread)			// 현재 스레드가 idle 스레드가 아닌 경우
		list_insert_ordered(&ready_list, &curr->elem, cmp_thread_priority, NULL);	// 스레드를 ready_list에 추가
